...
static _st_netfd_t *_st_netfd_new(int osfd, int nonblock, int is_socket)
{
    ...
    if (nonblock) {
        ...
        if (is_socket && nsa_ioctl(osfd, FIONBIO, &flags) != -1)
            return fd;
        /* Do it the Posix way */
        if ((flags = nsa_fcntl(osfd, F_GETFL, 0)) < 0 ||
            nsa_fcntl(osfd, F_SETFL, flags | O_NONBLOCK) < 0) {
            st_netfd_free(fd);
            return NULL;
        }
    }
...
_st_netfd_t *st_accept(_st_netfd_t *fd, struct sockaddr *addr, int *addrlen, st_utime_t timeout)
{
    ...
    while ((osfd = nsa_accept(fd->osfd, addr, (socklen_t *)addrlen)) < 0) {
        if (errno == EINTR)
            continue;
...
int st_netfd_serialize_accept(_st_netfd_t *fd)
{
    ...
    if ((p[0] = st_netfd_open(osfd[0])) != NULL && (p[1] = st_netfd_open(osfd[1])) != NULL && nsa_write(osfd[1], " ", 1) == 1) {
        fd->aux_data = p;
        return 0;
    }
    /* Error */
...
_st_netfd_t *st_accept(_st_netfd_t *fd, struct sockaddr *addr, int *addrlen, st_utime_t timeout)
{
    ...
    for ( ; ; ) {
        if (p == NULL) {
            osfd = nsa_accept(fd->osfd, addr, (socklen_t *)addrlen);
        } else {
            /* Get the lock */
...
int st_connect(_st_netfd_t *fd, const struct sockaddr *addr, int addrlen, st_utime_t timeout)
{
    ...
    while (nsa_connect(fd->osfd, addr, addrlen, NULL, 0) < 0) {
        if (errno != EINTR) {
            /*
            ...
            n = sizeof(int);
            if (nsa_getsockopt(fd->osfd, SOL_SOCKET, SO_ERROR, (char *)&err, (socklen_t *)&n) < 0)
                return -1;
            if (err) {
                errno = err;
                return -1;
...
ssize_t st_read(_st_netfd_t *fd, void *buf, size_t nbyte, st_utime_t timeout)
{
    ...
    while ((n = nsa_read(fd->osfd, buf, nbyte)) < 0) {
        if (errno == EINTR)
            continue;
        if (!_IO_NOT_READY_ERROR)
            return -1;
...
int st_readv_resid(_st_netfd_t *fd, struct iovec **iov, int *iov_size, st_utime_t timeout)
{
    ...
    while (*iov_size > 0) {
        if (*iov_size == 1)
            n = nsa_read(fd->osfd, (*iov)->iov_base, (*iov)->iov_len);
        else
...
int st_writev_resid(_st_netfd_t *fd, struct iovec **iov, int *iov_size, st_utime_t timeout)
{
    ...
    while (*iov_size > 0) {
        if (*iov_size == 1)
            n = nsa_write(fd->osfd, (*iov)->iov_base, (*iov)->iov_len);
        else
...
int st_recvfrom(_st_netfd_t *fd, void *buf, int len, struct sockaddr *from, int *fromlen, st_utime_t timeout)
{
    ...
    while ((n = nsa_recvfrom(fd->osfd, buf, len, 0, from, (socklen_t *)fromlen)) < 0) {
        if (errno == EINTR)
            continue;
        if (!_IO_NOT_READY_ERROR)
            return -1;
...
int st_sendto(_st_netfd_t *fd, const void *msg, int len, const struct sockaddr *to, int tolen, st_utime_t timeout)
{
    ...
    while ((n = nsa_sendto(fd->osfd, msg, len, 0, to, tolen)) < 0) {
        if (errno == EINTR)
            continue;
        if (!_IO_NOT_READY_ERROR)
            return -1;
...
int st_recvmsg(_st_netfd_t *fd, struct msghdr *msg, int flags, st_utime_t timeout)
{
    ...
    while ((n = nsa_recvmsg(fd->osfd, msg, flags)) < 0) {
        if (errno == EINTR)
            continue;
        if (!_IO_NOT_READY_ERROR)
            return -1;
...
int st_sendmsg(_st_netfd_t *fd, const struct msghdr *msg, int flags, st_utime_t timeout)
{
    ...
    while ((n = nsa_sendmsg(fd->osfd, msg, flags)) < 0) {
        if (errno == EINTR)
            continue;
        if (!_IO_NOT_READY_ERROR)
            return -1;
...
_st_netfd_t *st_open(const char *path, int oflags, mode_t mode)
{
    ...
    while ((osfd = nsa_open(path, oflags | O_NONBLOCK, mode)) < 0) {
        if (errno != EINTR)
            return NULL;
    }
