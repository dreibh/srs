...
ST_HIDDEN void _st_select_dispatch(void)
{
    ...
    nfd = nsa_select(_ST_SELECT_MAX_OSFD + 1, rp, wp, ep, tvp);
...
ST_HIDDEN void _st_poll_dispatch(void)
{
    ...
    /* Check for I/O operations */
    nfd = nsa_poll(_ST_POLLFDS, _ST_POLL_OSFD_CNT, timeout);
...
ST_HIDDEN int _st_kq_init(void)
{
    ...
    nsa_fcntl(_st_kq_data->kq, F_SETFD, FD_CLOEXEC);
...
ST_HIDDEN int _st_epoll_init(void)
{
    ...
    if ((_st_epoll_data->epfd = nsa_epoll_create(_st_epoll_data->fd_hint)) < 0) {
        err = errno;
        rv = -1;
        goto cleanup_epoll;
    }
    nsa_fcntl(_st_epoll_data->epfd, F_SETFD, FD_CLOEXEC);
...
ST_HIDDEN void _st_epoll_pollset_del(struct pollfd *pds, int npds)
{
    ...
    for (pd = pds; pd < epd; pd++) {
        ...
        if (events != old_events && _ST_EPOLL_REVENTS(pd->fd) == 0) {
            ...
            ev.events = events;
            ev.data.fd = pd->fd;
            if (nsa_epoll_ctl(_st_epoll_data->epfd, op, pd->fd, &ev) == 0 && op == EPOLL_CTL_DEL) {
...
ST_HIDDEN int _st_epoll_pollset_add(struct pollfd *pds, int npds)
{
    ...
    for (i = 0; i < npds; i++) {
        ...
        if (events != old_events) {
            ...
            ev.events = events;
            ev.data.fd = fd;
            if (nsa_epoll_ctl(_st_epoll_data->epfd, op, fd, &ev) < 0 && (op != EPOLL_CTL_ADD || errno != EEXIST))
                break;
            if (op == EPOLL_CTL_ADD) {
                _st_epoll_data->evtlist_cnt++;
...
ST_HIDDEN void _st_epoll_dispatch(void)
{
    ...
    if (_st_epoll_data->pid != getpid()) {
        ...
        nsa_close(_st_epoll_data->epfd);
        _st_epoll_data->epfd = nsa_epoll_create(_st_epoll_data->fd_hint);
        if (_st_epoll_data->epfd < 0) {
            /* There is nothing we can do here, will retry later */
            return;
        }
    ...
    /* Check for I/O operations */
    nfd = nsa_epoll_wait(_st_epoll_data->epfd, _st_epoll_data->evtlist, _st_epoll_data->evtlist_size, timeout);
    ...
    if (nfd > 0) {
        ...
        for (i = 0; i < nfd; i++) {
            ...
            ev.events = events;
            ev.data.fd = osfd;
            if (nsa_epoll_ctl(_st_epoll_data->epfd, op, osfd, &ev) == 0 && op == EPOLL_CTL_DEL) {
                _st_epoll_data->evtlist_cnt--;
            }
        }
    }
