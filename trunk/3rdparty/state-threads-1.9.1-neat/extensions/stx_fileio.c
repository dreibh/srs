...
file_reader(int fd, int *fd_control, int *fd_out)
{
    ...
    if (nsa_pipe(control_pipe) < 0 || nsa_pipe(out_pipe) < 0)
        return (pid_t)-1;

    pid = fork();
    if (pid == (pid_t) -1)
    {
        nsa_close(control_pipe[0]);
        nsa_close(control_pipe[1]);
        nsa_close(out_pipe[0]);
        nsa_close(out_pipe[1]);
        return pid;
    }
    else if (pid == (pid_t) 0)
    {
        ...
        while (sizeof(cb) == nsa_read(control_pipe[0], &cb, sizeof(cb))) {
            ssize_t nb;
            if (0 >= cb.nbytes)
                goto clean_exit;
            if (pos != cb.offset) {
                pos = nsa_lseek(fd, cb.offset, SEEK_SET);
                if (pos == (off_t)-1)
                    break;
            }
            nb = nsa_read(fd, buf, cb.nbytes);
            if (nb == (ssize_t)-1)
                break;
            pos += nb;
            nsa_write(out_pipe[1], (char *)&nb, sizeof(nb));
            nsa_write(out_pipe[1], buf, nb);
        }
